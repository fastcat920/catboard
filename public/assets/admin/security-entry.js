(function () {
    ...
    function clientPolicyForm(node, policy) {
        ...
        return '<form class="node-entry-client-policy" data-policy-id="' + (policy.id || "") + '"><div class="node-entry-grid">' +
            ...
            '<label>下发模式<select name="delivery_mode"><option value="primary_only"' + (policy.delivery_mode === "primary_only" ? " selected" : "") + '>仅主入口</option><option value="manual_backup"' + (policy.delivery_mode === "manual_backup" ? " selected" : "") + '>主入口＋备用入口</option><option value="auto_fallback"' + ((!policy.delivery_mode || policy.delivery_mode === "auto_fallback") ? " selected" : "") + '>自动故障转移</option></select></label>' +
    ...
    function renderEntryPoolModal(node, editingId, editingPolicyId) {
        ...
        modal.innerHTML = '<div class="node-entry-pool-box"><div class="node-entry-pool-head"><div><b>管理节点入口 · ' + escapeHtml(node.server_name) + '</b><small>节点 ID ' + node.server_id + ' · ' + escapeHtml(node.server_type) + ' · 原地址 ' + escapeHtml(node.original_address) + '</small></div><button type="button" data-entry-close>×</button></div>' +
            ...
            '<section><h3>客户端 UA 下发规则</h3><p class="muted">命中规则时覆盖上方默认设置；数字越小优先级越高。UA 可伪造，规则不会扩大用户的节点权限。</p><div class="node-entry-table-wrap"><table><thead><tr><th>客户端</th><th>平台 / 版本</th><th>下发模式</th><th>优先级</th><th>状态</th><th>操作</th></tr></thead><tbody>' + (policies.length ? policies.map(function (policy) { var versionRange = (policy.min_version ? "≥ " + policy.min_version : "") + (policy.min_version && policy.max_version ? "，" : "") + (policy.max_version ? "≤ " + policy.max_version : ""); return '<tr><td>' + escapeHtml(policy.client_family === "*" ? "全部客户端" : policy.client_family) + '</td><td>' + escapeHtml(!policy.client_platform || policy.client_platform === "*" ? "全部平台" : policy.client_platform) + '<br><span class="muted">' + escapeHtml(versionRange || "全部版本") + '</span></td><td>' + escapeHtml({ primary_only: "仅主入口", manual_backup: "主入口＋备用入口", auto_fallback: "自动故障转移" }[policy.delivery_mode] || policy.delivery_mode) + '</td><td>' + policy.priority + '</td><td>' + (policy.enabled ? "启用" : "停用") + '</td><td><button type="button" data-policy-edit="' + policy.id + '">编辑</button> <button type="button" class="danger" data-policy-delete="' + policy.id + '">删除</button></td></tr>'; }).join("") : '<tr><td colspan="6" class="empty">尚未添加客户端规则，所有客户端使用默认下发设置</td></tr>') + '</tbody></table></div><h3>' + (editingPolicy ? "编辑客户端规则" : "添加客户端规则") + '</h3>' + clientPolicyForm(node, editingPolicy) + '</section>' +
    ...
    function bindEntryPoolModal(modal, node) {
        ...
        modal.querySelector(".node-entry-client-policy").onsubmit = function (event) {
            ...
            api("/security/entry-client-policy/save", { method: "POST", body: JSON.stringify({
                ...
                delivery_mode: form.elements.delivery_mode.value, priority: Number(form.elements.priority.value),
