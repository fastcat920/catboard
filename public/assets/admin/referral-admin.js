(function () {
    ...
    function renderShell() {
            root.innerHTML = '<div class="p-0 p-lg-4"><div class="mb-0 block border-bottom"><nav class="nav nav-tabs nav-tabs-block">' + [["overview","数据概览"],["funnel","数据漏斗"],["setting","奖励规则"],["levels","推广等级"],["milestones","里程碑"],["campaigns","邀请活动"],["materials","分享素材"],["leaderboard","排行榜"],["relations","邀请关系"],["rewards","奖励流水"]].map(function (item) {
    ...
    function loadTab() {
        ({ overview: loadOverview, funnel: loadFunnel, setting: loadSetting, levels: loadLevels, milestones: loadMilestones, campaigns: loadCampaigns, materials: loadMaterials, leaderboard: loadLeaderboard, relations: loadRelations, rewards: loadRewards }[tab] || loadOverview)();
    }
    ...
    function loadSetting() {
        ...
        cachedApi("/dashboard").then(function (payload) { if (tab !== requestedTab) return; var s = payload.data.setting || {};
            content('<div class="block block-rounded"><div class="block-header block-header-default"><h3 class="block-title">基础奖励规则</h3></div><div class="block-content"><form data-setting class="referral-form"><div class="custom-control custom-switch mb-4"><input class="custom-control-input" id="referral-enabled" name="enabled" type="checkbox" ' + (s.enabled ? "checked" : "") + '><label class="custom-control-label" for="referral-enabled">启用新版邀请计划</label></div>' +
                ...
                '<div class="form-group"><label>新人首单优惠券模板</label><select class="form-control" name="newcomer_coupon_id"><option value="">不发放优惠券</option>'+(payload.data.coupon_templates||[]).map(function(c){return '<option value="'+c.id+'" '+(String(s.newcomer_coupon_id||'')===String(c.id)?'selected':'')+'>'+esc(c.name)+'（'+esc(c.code)+'）</option>';}).join('')+'</select><small class="form-text text-muted">受邀用户注册后生成仅限本人使用的一次性券码。</small></div>'+field("newcomer_reward_valid_days", "新人奖励有效期（天）", s.newcomer_reward_valid_days||30)+
    ...
    function loadFunnel(){api('/funnel?days=30').then(function(p){if(tab!=='funnel')return;var s=p.data.summary,t=p.data.trend||[];content('<div class="referral-cards">'+[['链接访问',s.visits],['邀请注册',s.registrations],['邮箱验证',s.verified===null?'未启用验证':s.verified],['首购订单',s.first_orders],['续费用户',s.renewal_users],['持续续费率',s.renewal_rate+'%'],['待确认奖励',s.pending_rewards],['已撤销奖励',s.reversed_rewards],['奖励投入产出比',s.roi===null?'-':s.roi]].map(function(x){return '<section class="block block-rounded"><div class="block-content"><small>'+x[0]+'</small><strong>'+x[1]+'</strong></div></section>';}).join('')+'</div><div class="row mt-3"><div class="col-lg-7">'+table(['日期','访问','注册','首购','续费'],t.slice(-30).reverse().map(function(x){return [x.date,x.visits,x.registrations,x.first_orders,x.renewals];}))+'</div><div class="col-lg-5"><h5>渠道转化排行</h5>'+table(['渠道','访问','注册'],(p.data.channels||[]).map(function(x){return [x.channel,x.visits,x.registrations];}))+'<h5 class="mt-4">套餐转化排行</h5>'+table(['套餐 ID','订单','收入'],(p.data.plans||[]).map(function(x){return [x.plan_id,x.orders,'¥'+money(x.revenue)];}))+'<h5 class="mt-4">活动转化数据</h5>'+table(['活动','发放次数','奖励支出'],(p.data.campaigns||[]).map(function(x){return [x.name,x.granted_count,'¥'+money(x.spent_amount)];}))+'</div></div>');}).catch(fail);}
    function loadMaterials(){cachedApi('/materials').then(function(p){if(tab!=='materials')return;content('<div class="block block-rounded"><div class="block-header block-header-default"><h3 class="block-title">分享海报与文案</h3><button class="btn btn-sm btn-primary" data-add>新增素材</button></div><div class="block-content p-0">'+table(['排序','名称','中文文案','英文文案','海报','状态','操作'],p.data.map(function(x){return [x.sort,x.name,x.copy_zh,x.copy_en,x.image_data?'已上传':'未上传',x.enabled?'启用':'停用',''];}))+'</div></div>');root.querySelector('[data-add]').onclick=function(){editMaterial(null);};root.querySelectorAll('tbody tr').forEach(function(tr,i){var row=p.data[i];if(!row)return;tr.lastElementChild.innerHTML='<button class="btn btn-sm btn-light" data-edit>编辑</button><button class="btn btn-sm btn-light text-danger" data-drop>删除</button>';tr.querySelector('[data-edit]').onclick=function(){editMaterial(row);};tr.querySelector('[data-drop]').onclick=function(){if(confirm('确认删除素材？'))api('/material/drop',{method:'POST',body:JSON.stringify({id:row.id})}).then(function(){clearCache('/materials');loadMaterials();}).catch(function(e){alert(e.message);});};});}).catch(fail);}
    function editMaterial(row){row=row||{};var modal=document.createElement('div');modal.className='referral-modal';modal.innerHTML='<div class="referral-modal-dialog referral-modal-lg"><div class="referral-modal-head"><h3>'+(row.id?'编辑':'新增')+'分享素材</h3><button data-cancel>×</button></div><form><div class="referral-modal-body"><div class="row"><div class="col-md-6"><label>中文名称</label><input class="form-control" name="name" required value="'+esc(row.name||'')+'"></div><div class="col-md-6"><label>英文名称</label><input class="form-control" name="name_en" value="'+esc(row.name_en||'')+'"></div></div><div class="row mt-3"><div class="col-md-6"><label>中文分享文案</label><textarea class="form-control" rows="4" name="copy_zh">'+esc(row.copy_zh||'')+'</textarea></div><div class="col-md-6"><label>英文分享文案</label><textarea class="form-control" rows="4" name="copy_en">'+esc(row.copy_en||'')+'</textarea></div></div><div class="form-group mt-3"><label>海报模板（PNG/JPG/WebP，最大约 2MB）</label><input class="form-control-file" type="file" accept="image/png,image/jpeg,image/webp" name="image"><input type="hidden" name="image_data" value=""><small>'+(row.image_data?'已上传模板；不选择文件将保留原图':'尚未上传')+'</small></div>'+field('sort','排序',row.sort||0)+'<div class="custom-control custom-switch"><input class="custom-control-input" id="material-enabled" name="enabled" type="checkbox" '+(row.enabled===0?'':'checked')+'><label class="custom-control-label" for="material-enabled">启用素材</label></div></div><div class="referral-modal-foot"><button class="btn btn-light" type="button" data-cancel>取消</button><button class="btn btn-primary" type="submit">保存</button></div></form></div>';root.appendChild(modal);var imageData=null,file=modal.querySelector('[name="image"]');file.onchange=function(){if(!file.files[0])return;if(file.files[0].size>2200000){alert('图片不能超过 2MB');file.value='';return;}var reader=new FileReader();reader.onload=function(){imageData=reader.result;};reader.readAsDataURL(file.files[0]);};modal.querySelectorAll('[data-cancel]').forEach(function(b){b.onclick=function(){modal.remove();};});modal.querySelector('form').onsubmit=function(e){e.preventDefault();var data={name:e.target.name.value.trim(),name_en:e.target.name_en.value.trim(),copy_zh:e.target.copy_zh.value.trim(),copy_en:e.target.copy_en.value.trim(),image_data:imageData===null?(row.image_data||null):imageData,sort:Number(e.target.sort.value||0),enabled:e.target.enabled.checked?1:0};if(row.id)data.id=row.id;api('/material/save',{method:'POST',body:JSON.stringify(data)}).then(function(){clearCache('/materials');modal.remove();loadMaterials();}).catch(function(error){alert(error.message);});};}
