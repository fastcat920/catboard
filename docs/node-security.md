...
节点加入监控后必须至少一次达到“TCP 双向正常”，才会建立首次监控正常基线并允许后续异常自动创建事件。从未正常的节点仍会持续探测并展示判断结果，但不自动创建事件。事件候选窗口从“首次监控正常时间”和“首次失败时间减去封锁关联窗口”两者中较晚的时间开始，结束于首次失败时间。移除后重新添加节点会开始新的监控周期并重新建立基线。
