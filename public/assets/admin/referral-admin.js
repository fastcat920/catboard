(function () {
    ...
    function loadLevels() { cachedApi("/levels").then(function (p) { if(tab !== "levels") return; renderRuleTable("推广等级（长期权益）", p.data, "level", ["名称","升级条件","返佣比例","套餐优惠"], function(x){return [x.name,x.required_invites+" 人 + ¥"+money(x.required_revenue),x.commission_rate+"%",x.member_discount?"减免 "+x.member_discount+"%":"无"];}); }).catch(fail); }
